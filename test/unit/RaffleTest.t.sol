...
contract RaffleTest is Test, CodeConstants {
    ...
    function testRaffleRevertWhenDontPlayEnough() public {
        vm.prank(PLAYER);
    ...
    function testCheckUpkeepReturnsFalseIfTimeHasNotYetPassed() public {
        vm.prank(PLAYER);
        raffle.enterRaffle{value: entranceFee}();
    ...
    /* Perform Up Keep */
    function testPerformUpKeepWorksWhenCheckUpkeepIsTrue() public {
        vm.prank(PLAYER);
        raffle.enterRaffle{value: entranceFee}();
        vm.warp(block.timestamp + interval + 1);
        vm.roll(block.number + 1);
    ...
    function testPerformUpKeepUpdatesRaffleStateAndUpdatesRequestId() public raffleEntered {
        vm.recordLogs();
        raffle.performUpkeep("");
